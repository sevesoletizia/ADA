...
The questions and the data will be made available to you at 16:15 on Tuesday January 13th, 2026.
...
## Environment
We have prepared a python environment at [requirements.txt](https://github.com/epfl-ada/2025/blob/main/Exam/requirements.txt) with all the Python packages that you might need for the exam. Please prepare and test that your environment is working by executing the cells in [exam_dryrun.ipynb](https://github.com/epfl-ada/2025/blob/main/Exam/exam_dryrun.ipynb).
...
The exam will take place in the following rooms: CE11, CE12, CE13, CE14, CE15, CE16, CO1, CO2, CO3.
...
2. Set up the [environment](#Environment).
3. Execute all the cells of `exam_dryrun.ipynb` in Jupyter with your environment activated. On successful execution, it should print **'Package import test successful!'** without throwing any errors.
4. Make sure you can access the [Google document](https://docs.google.com/document/d/e/2PACX-1vQWG_J3UfU8WaVCly1HpPwCSf49sDHT6hjOQUSSFoJasENqUq_x2HHpqQoJ5m66gt2RmSJ0q9-ZJMQD/pub) containing the *"Announcements"* section. This document would be used for sharing the announcements during the exam.
...
9.  Fully read the instructions for each question before starting to solve it to avoid misunderstandings, and remember to save your notebook regularly.
10. We will **not run your notebook for you**! Rather, we will grade it as is, which means that only the results contained in your evaluated code cells will be considered, and we will not see the results in unevaluated code cells. Thus, be sure to hand in a **fully run and evaluated notebook**.
11. In continuation to the previous point, interactive plots, such as those generated using `plotly`, should be **strictly avoided**!
